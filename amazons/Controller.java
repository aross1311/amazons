...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("([a-j]([1-9]|10)-[a-j]([1-9]|10)\\([a-j]([1-9]|10)\\))$"
                + "|([a-j]([1-9]|10)\\s+){2}[a-j]([1-9]|10)$", this::doMove),
        new Command("auto\\s+(black|white)$", this::setAutoPlayer),
        new Command("manual\\s+(black|white)$", this::setManualPlayer)
    ...
    /** Do the move as passed in from the command line from MAT. */
    private void doMove(Matcher mat) {
        _board.makeMove(Move.mv(mat.group()));
        if (_board.winner() != EMPTY) {
            _winner = _board.winner();
            String winner = "White";
            if (_winner == BLACK) {
                winner = "Black";
            }
            _reporter.reportNote(winner + " wins.");
        }
    }

    /** Set MAT to be an AI. */
    private void setAutoPlayer(Matcher mat) {
    ...
    /** set MAT to be a manual player. */
    private void setManualPlayer(Matcher mat) {
        if (mat.group(1).equals("white")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else {
